...
- Deep links на матчи (`/match/:id`, `/uk/match/:id`) не пререндерятся. Нельзя отдавать Cloudflare SPA `index.html` — там payload домашней страницы и Nuxt уводит на `/`. Вместо этого `workers/spa-shell.js` отдаёт Nuxt `200.html` (клиентский shell) для document-запросов без файла.
